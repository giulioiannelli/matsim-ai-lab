...
//	});
  ...
  private LLMConfigGroup buildLLMConfig() {
      ...
      config.setBackend(BackendType.OPENAI_COMPAT);
      ...
      switch (this.backend) {
          // All three labels map to OPENAI_COMPAT — they differ only in whether
          // HTTPS is used. The "ollama_native" label below selects Ollama's
          // richer /api/chat endpoint.
          case "openai" -> {
              config.setBackend(LLMConfigGroup.BackendType.OPENAI_COMPAT);
          ...
          case "ollama_native" -> {
              config.setBackend(LLMConfigGroup.BackendType.OLLAMA_NATIVE);
              config.setUseHttps(false);
          }
      }
